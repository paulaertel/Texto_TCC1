...
Seja $A \in \RR^{n\times n}$ uma matriz simétrica. Dizemos que $A$ é \emph{definida positiva} quando $x^{T}Ax >0$, para todo $x\in \RR^{n}$. Tal propriedade é denotada por $A \succ 0$. Se $x^{T}Ax \geq 0$, para todo $x\in \RR^{n}\backslash\{ 0 \}$, $A$ é dita \emph{semidefinida positiva}, fato este denotado por $A \succcurlyeq 0$.
...
Suponhamos agora que $\nabla f(\xbar)^{T}d <0$. Temos que, pelo \Cref{fato:Taylor_primeira_ordem},
\[
