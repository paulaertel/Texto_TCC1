...
%-------------------------------------------------------------------------------------------------------------------
\subsection{Minimização com Restrições Lineares de Desigualdade}
...
\begin{defi}
Dado $\xbar \in \Omega$, dizemos que uma restrição de desigualdade $a_{i}^{T}\bar{x} \leq b_{i}$ é \emph{ativa} em $\xbar$ quando $a_{i}^{T}\bar{x} = b_{i}$. Caso $a_{i}^{T}\bar{x} < b_{i}$, dizemos que a restrição é inativa em $\xbar$. 
\end{defi}
E seja $\mathcal{M} \coloneqq \{ 1,2, \ldots , m \} $, o conjunto dos índices das restrições de desigualdade ativas é denotado por
\[
\mathcal{I} (\xbar) = \{ i \in \mathcal{M} \mid a_{i}^{T}\bar{x} = b_{i} \} .
\]

Além disso, a cada $x\in \Omega$ podemos associar um número $r(x)$, com $0\leq r(x) \leq m$, que representa a quantidade de restrições ativas em $x$. É importante salientar que o conjunto das direções factíveis a partir de $x$ depende das restrições ativas nesse ponto.
Assim, considere $x\in \Omega$ tal que $r(x) = p$ com $0<p \leq m$. O \Cref{teo:direcoes_factiveis_restricao_desigualdade} nos fornece uma caracterização das direções factíveis em $x$.

\begin{teo} \label{teo:direcoes_factiveis_restricao_desigualdade}
$d \in \RR^{n}$ é factível em $x$ se, e somente se, $a_{i}^{T}d \leq 0$ para todo $i \in \mathcal{I} (x)$.
\end{teo}
\begin{proof}
