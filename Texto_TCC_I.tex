...
\begin{figure}[!h] 
	\centering
	\includegraphics[width=0.80\textwidth]{grafico_minimo_local_global}
...
\[
\Omega = \{ x\in \RR^{n} \mid a_{i}^{T}x \leq b_{i} \ \text{para todo} \ i \in \{ 1,2, \ldots , m \} \} .
\]
...
Um conceito fundamental para formular as condições de otimalidade e que fornece uma grande quantidade de informações é o de \emph{restrição ativa}.
...
\begin{figure}[!h] 
	...
	\caption{Restrições ativas e inativas. \\ Fonte: \textcite{luenberger2008linear} \label{fig:restricoes_inativas}}
\end{figure}

Ademais, a cada $x\in \Omega$ podemos associar um número $r(x)$, com $0\leq r(x) \leq m$, que representa a quantidade de restrições ativas em $x$. O \Cref{teo:direcoes_factiveis_restricao_desigualdade} nos fornece uma caracterização das direções factíveis em $x$.
...
\]
Usando \eqref{eq:2_condicao_necessaria_1ordem_restricao_desigualdade} e como $\lambda_{j} >0$, obtemos
\[
a_{i_{j}}^{T} d <0.
\]
