...
de dados Iris e o pacote \texttt{DataFrame} para gerar as tabelas de
dados.
...
\texttt{RDatasets}. Perceba que as amostras pertencem a três diferentes
espécies: setosa, versicolor e virginica. Porém, como nossos estudos se
concentram na classificação binária, nosso objetivo inicial será
aprender a separar os dados em setosa e não setosa e posteriormente em
virginica e não virginica.
...
\end{tcolorbox}
    ...
    Nos gráficos abaixo, os pontos em azul pertencem à espécie setosa, os
pontos em vermelho à espécie versicolor e os pontos em verde à espécie
virginica. Eles foram construídos considerando-se duas características
de cada vez.
...
% \prompt{Out}{outcolor}{3}{}
    ...
    \begin{center}
    \includegraphics[width=0.7\textwidth]{Implementacao_de_SVM_em_Julia_TCC_files/Implementacao_de_SVM_em_Julia_TCC_6_0.pdf}
    \end{center}
...
De acordo com o Capítulo 5, a técnica SVM com margem rígida pode ser
formulada pelo problema (74). Assim, utilizando esta formulação,
apresentamos a seguir uma aplicação na classificação do conjunto de
dados Íris em setosa e não setosa com base em duas características
apenas.
...
de entrada \(x^{i}\) pertencem ao espaço \(\mathbb{R}^{2}\). Logo,
...
{            
    \prompt{Out}{outcolor}{6}{}
        ...
        \begin{center}
        \includegraphics[width=0.7\textwidth]{Implementacao_de_SVM_em_Julia_TCC_files/Implementacao_de_SVM_em_Julia_TCC_12_0.pdf}
        \end{center}
    }    
...
de treinamento que o algoritmo SVM irá ``aprender'' a classificar os
...
\prompt{Out}{outcolor}{9}{}
    ...
    \begin{center}
    \includegraphics[width=0.7\textwidth]{Implementacao_de_SVM_em_Julia_TCC_files/Implementacao_de_SVM_em_Julia_TCC_18_0.pdf}
    \end{center}
    % { \hspace*{\fill} \\}
    }

    Portanto, no gráfico acima a reta magenta corresponde ao hiperplano
ótimo e as retas tracejadas são os hiperplanos que delimitam a máxima
margem possível. Observe que o conjuto de dados de treinamento é
linearmente separável, pois o hiperplano ótimo os está separando
corretamente.

Além disso, de acordo com a Definição 14 do Capítulo 5.2, temos que os
...
{\prompt{Out}{outcolor}{10}{}
     ...
     \begin{center}
     \includegraphics[width=0.7\textwidth]{Implementacao_de_SVM_em_Julia_TCC_files/Implementacao_de_SVM_em_Julia_TCC_20_0.pdf}
     \end{center}
     % { \hspace*{\fill} \\}
    } 
...
Note que, quanto mais próxima de 1 for a acurácia, mais assertivo é o
classificador.
...
\end{tabular}
    ...
    A tabela acima corresponde à matriz de confusão para o hiperplano ótimo
encontrado, em que dos 100 dados do conjunto de teste, o número de $VP =
...
\prompt{Out}{outcolor}{13}{}
    ...
    \begin{center}
    \adjustimage{max size={0.6\linewidth}{0.9\paperheight}}{Implementacao_de_SVM_em_Julia_TCC_files/Implementacao_de_SVM_em_Julia_TCC_26_0.pdf}
    \end{center}
    }
...
{\prompt{Out}{outcolor}{15}{}
    ...
    \begin{center}
    \includegraphics[width=0.7\textwidth]{Implementacao_de_SVM_em_Julia_TCC_files/Implementacao_de_SVM_em_Julia_TCC_30_0.pdf}
    \end{center}
    }
...
setosa serão classificados como 1, e os demais, pertencetes às espécies
...
\end{tcolorbox}
    ...
    Analisando os gráficos da firgura (referenciar pairplot) temos que os
...
\(x^{i} \in \mathbb{R}^{4}\). Mas gora, dados da espécie virginica serão
classificados com \(y_{i} = 1\) e dados das espécies não virginica com
\(y_{i} = -1\).
...
\end{tcolorbox}
    ...
    Observando novamente os gráficos apresentados em (referenciar pairplot),
...
\end{tcolorbox}
    ...
    Agora, para obter o hiperplano ótimo aplicamos a função
\texttt{SVM\_flexivel} com parâmetro \(C = 1\).
...
{            
    \prompt{Out}{outcolor}{27}{}
        ...
        \begin{center}
        \includegraphics[width=0.7\textwidth]{Implementacao_de_SVM_em_Julia_TCC_files/Implementacao_de_SVM_em_Julia_TCC_54_0.pdf}
        \end{center}
    }    
...
\end{tcolorbox}
    ...
    Neste exemplo, cada dado \(x^{i}\) do conjunto de entrada pertence ao
\(\mathbb{R}^{30}\), pois são 30 as características. Ademais, os dados
diagnosticados como malignos (M) serão classificados como \(y_{i} = 1\),
e os dados com diagnóstico benigno serão classificados com
\(y_{i} = -1\).

Com base nos gráficos apresentados anteriormente em (referenciar
pairplot), em que apenas duas características foram consideradas em cada
...
\end{tcolorbox}
    ...
    Utilizando a função \texttt{SVM\_flexivel} criada com base na modelagem
matemática desenvolvida no Capítulo 5.3, determinamos o hiperplano ótimo
para os seguintes valores para o parâmetro \(C\):
\(10^{-3}, 10^{-2}, 10^{-1} , 1, 10, 10^{2}, 10^{3}\) e \(10^{4}\). Para
...
\(C\) a solução ótima também varia, porém nenhuma delas apresenta 100\%
de acerto na classificação dos dados de teste. Em vista disso, para
