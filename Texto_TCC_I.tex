...
Logo, definindo $\wbar:=\dfrac{w}{\boldsymbol{\gamma}}$ e $\bbar :=\dfrac{b}{\boldsymbol{\gamma}}$, obtemos as desigualdades \eqref{eq1} e \eqref{eq2}. 
...
\subsection{Condições de otimalidade para problemas sem restrições}
...
Para demonstrar as condições necessárias utilizaremos a expansão de Taylor de primeira e segunda ordem.
...
f(x^{*}) \leq f(x^{*}+td),
\]
para todo $t\in (0,\varepsilon ]$. Pela diferenciabilidade de $f$ em $x^{*}$, aplicando a expansão de Taylor \eqref{teo:Taylor_primeira_ordem}, 
\[
f(x^{*}+td) = f(x^{*}) + t\nabla f(x^{*})^{T}d + o(t),
\] 
...
\end{teo}
\begin{proof}
Seja $d\in \RR^{n}$ arbitrário, porém fixo. Se $x^{*}$ é minimizador local de $f$, então para todo $t>0$ suficientemente pequeno,
\[\label{eq:1_condicao_2_ordem_irrestrito}
0 \leq f(x^{*}+td) - f(x^{*}).
\]
Aplicando a expansão de Taylor de segunda ordem \eqref{teo:Taylor_segunda_ordem},
\[
f(x^{*}+td) = f(x^{*}) + t\nabla f(x^{*})^{T}d + \dfrac{t^{2}}{2} d^{T}\nabla^{2} f(x^{*})d + o(t^{2}),
\]
que por \eqref{eq:1_condicao_2_ordem_irrestrito} implica em
\[ \label{eq:2_condicao_2_ordem_irrestrito}
0 \leq \dfrac{t^{2}}{2} d^{T}\nabla^{2} f(x^{*})d + o(t^{2}),
\]
com $\lim_{t\rightarrow 0} \dfrac{o(t^{2})}{t^{2}} =0$, pois como $x^{*}$ é minimizador local, o \Cref{teo:condicao_necessaria_1_ordem} garante que $\nabla f(x^{*}) =0$. Dividindo ambos os lados da desigualdade \eqref{eq:2_condicao_2_ordem_irrestrito} por $t^{2}$ e tomando o limite quando $t\rightarrow 0$, obtemos
\[
d^{T} \nabla^{2} f(x^{*})d \geq 0.
\]
\end{proof}
...
\begin{teo}(\textbf{Condição suficiente de segunda ordem}) \label{tep:condicao_suficiente_irrestrito}
...
\phi (h) = h^{T} \nabla^{2} f(x^{*})h.
\]
A função $\phi$ é contínua e $B$ é um conjunto fechado e limitado, portanto, pelo \Cref{teo:weierstrass}, $\phi$ atinge um valor máximo e um valor mínimo em $B$. Considere $a$ o valor mínimo, então
\[
\phi(h) \geq a >0,
\]
...
\]
Aplicando a expansão de Taylor de segunda ordem \eqref{teo:Taylor_segunda_ordem} em torno de $x^{*}$, temos
...
f(x^{*}+d) - f(x^{*}) = \dfrac{1}{2} d^{T}\nabla^{2} f(x^{*})d + o(\Vert d \Vert^{2}) \geq \dfrac{a}{2}\Vert d \Vert^{2} + o(\Vert d \Vert^{2}).
\]
...
\begin{prop}
$\boldsymbol{\gamma}^{*}$ é um minimizador local (global) de $\varphi$ em $\RR^{n-m}$ se, e somente se, $x^{*} := \tilde{x} + Z\boldsymbol{\gamma}^{*}$ é um minimizador local (global) de \eqref{problema_geral_minimizacao_igualdade}.
\end{prop}
\begin{proof}
...
\]
para todo $\boldsymbol{\gamma} \in \RR^{n-m}$. Logo, chamando $x^{*}:=\tilde{x} + Z\boldsymbol{\gamma}^{*}$, temos
\[
f(x^{*}) \leq f(x),
\]
