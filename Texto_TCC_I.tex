...
Neste capítulo abordaremos alguns conceitos e resultados de Otimização para o problema
...
As condições \eqref{eq:1_ponto_estacionario} são conhecidas como condições de Karush-Kuhn-Tucker (KKT). Assim, um ponto é dito estacionário quando satisfaz as condições KKT. Sob a hipótese de regularidade, as condições KKT são satisfeitas em um ponto que seja minimizador.

\begin{defi} \label{defi:ponto_regular}
Seja $x^{*}$ um ponto que satisfaça as restrições
\[
h(x^{*}) = 0, \quad g(x^{*}) \leq 0, \label{eq:1_ponto_regular}
\]
e seja $I(x^{*})$ o conjunto dos índices $i$ tais que $g_{i}(x^{*}) = 0$. Então $x^{*}$ é chamado de \emph{ponto regular} das restrições \eqref{eq:1_ponto_regular} se os gradientes das restrições de igualdade e desilgualdade ativas, isto é, $\nabla h_{j}(x^{*})$ e $\nabla g_{i}(x^{*})$, com $1 \leq j \leq m$ e $i \in I(x^{*})$, são linearmente independentes.
\end{defi}
...
\begin{teo}(\emph{KKT}) Seja $x^{*} \in \RR^{n}$ um minimizador local do problema \eqref{problema_geral_otimizacao} e suponha que $x^{*}$ é um ponto regular para as restrições. Então existem vetores $\mu^{*} \in \RR^{m}$ e $\lambda^{*} \in \RR^{p}$ tais que $(x^{*} , \mu^{*} , \lambda^{*} )$ cumpre \eqref{eq:1_ponto_estacionario}.
\end{teo}

É importante observar que se não for verificada a condição de regularidade podemos ter minimizadores que não cumprem KKT, o que dificulta a caracterização de tais pontos. Vejamos a seguir um exemplo que justifica essa afirmação.
...
Tendo em vista que em nosso problema o conjunto viável é poliedral, abordaremos nas seções seguintes como resolver o problema de minimização nesse contexto. Para tanto, analisaremos os problemas de minimização com restrições lineares de igualdade e, em seguida, de desigualdade, para posteriormente tratar do caso mais geral.


\subsection{Minimização com Restrições Lineares de Igualdade}
...
\end{aligned}
\]
em que $A \in \RR^{m\times n}$, $1 \leq m <n$ e posto $A=m$.

O conjunto
\[
S := \{ x \in \RR^{n} \mid Ax=b \}
\]
...
De modo geral, o conjunto de factibilidade $S$ é uma reta se $m=n-1$, um plano se $m=n-2$ e uma variedade de dimensão $n-m$ para $m$ genérico. É interessante perceber que se $n>3$ e $m=1$ pode-se dizer que $S$ é um hiperplano. 

Para definir as condições de otimalidade para o problema \eqref{problema_geral_minimizacao_igualdade}, é preciso primeiramente determinar o conjunto de direções factíveis em $S$. Para tanto, associado a $S$ temos o conjunto de soluções do sistema homogêneo $Ax=b$, que é chamado de Núcleo de $A$ e denotado por $\mathcal{N}(A)$. Este conjunto é um subespaço de $\RR^{n}$ de dimensão $n-m$, pois posto $A=m$. Ademais, é interessante observar que $\mathcal{N}(A)$ é um subespaço paralelo à variedade afim $S$ e passa pela origem. Esta noção é representada na \Cref{fig:nucleoA_paralelo_S}.
...
Como posto $A=m$, as $m$ linhas de $A$ formam um conjunto de vetores linearmente independentes que geram o subespaço Imagem de $A^{T}$, de dimensão $m$, que é denotado por $Im(A^{T})$. Dessa forma, as linhas de $A$ são ortogonais a $\mathcal{N}(A)$, ou, em outras palavras, $\mathcal{N}(A)$ é o complemento ortogonal de $Im(A^{T})$. 

\begin{prop} \label{prop:complemento_ortogonal_nucleo}
Seja $A \in \RR^{m\times n}$. $\mathcal{N}(A) = Im(A^{T})^{\bot}$.
\end{prop}
\begin{proof}
Um vetor $v \in \mathcal{N}(A)$ se, e somente se, $Av=0$. Mas isso ocorre se, e somente se, $Av$ é ortogonal a todo vetor $u\in \RR^{m}$, isto é, $u^{T}Av=0$, para todo $u \in \RR^{m}$. Assim, $(u^{T}Av)^{T} = 0^{T}$ implica que $v^{T}(A^{T}u) = 0$. Agora, variando $u \in \RR^{m}$, temos que $A^{T}u$ fornece o conjunto $Im(A^{T})$. Desse modo, $v^{T}(A^{T}v) = 0$, para todo $u \in \RR^{m}$, se, e somente se, $v \in Im(A^{T})^{\bot}$. Portanto, $v\in \mathcal{N}(A)$ se, e somente se, $v\in Im(A^{T})^{\bot}$.
\end{proof}

Em vista disso, os subespaços $\mathcal{N}(A)$ e $Im(A^{T})$ verificam 
\[ \mathcal{N}(A) \cap Im(A^{T}) = \{ 0 \} \]
e 
\[ \RR^{n} = \mathcal{N}(A) \oplus Im(A^{T}) .\]

A partir disso, se $d \in \mathcal{N}(A)$ e $\tilde{x}$ é uma solução da restrição \eqref{eq:restricao_de_igualdade}, então $x:= \tilde{x} + \alpha d$ também é solução de \eqref{eq:restricao_de_igualdade}, pois
\[
A(\tilde{x} + \alpha d) = A\tilde{x}
 + \alpha Ad = A\tilde{x} = b.
\]
Portanto, qualquer $d \in \mathcal{N}(A)$ é uma direção no espaço na qual podemos nos deslocar a partir de uma solução factível sem correr o risco de abandonar a região de factibilidade. 
...
A partir disso, podemos construir uma parametrização para caracterizar o conjunto factível. Se $\{ z^{1}, z^{2}, \ldots , z^{n-m} \}$ é uma base de $\mathcal{N}(A)$ e seja $Z$ a matriz de dimensão $n\times (n-m)$ cujas colunas são os vetores $z^{i}$, resulta que para todo $d \in \mathcal{N}(A)$, existe $\gamma \in \RR^{n-m}$ tal que $d=Z\gamma$, ou seja, $d$ é uma combinação linear dos vetores da base do núcleo de A. Assim, se $\tilde{x}$ é uma solução de \eqref{eq:restricao_de_igualdade}, então
\[ \label{parametrizacao_de_S}
S = \{ x \in \RR^{n} \mid x=\tilde{x} + Z\gamma , \gamma \in \RR^{n-m} \} .
\]

\subsubsection{Condições Necessárias de Primeira Ordem}

A caracterização de $S$ dada em \eqref{parametrizacao_de_S} permite definir a seguinte função $\varphi : \RR^{n-m} \rightarrow \RR$
\[ \label{funcao_phi_restricoes_igualdade}
\varphi (\gamma) = f(\tilde{x} + Z\gamma),
\]
e, assim, podemos considerar o seguinte problema de minimização sem restrições
\[ \label{problema_minimizacao_irrestrita_phi}
\min_{\gamma} \varphi (\gamma) .
\]

Este problema é obtido transferindo-se as restrições de \eqref{problema_geral_minimizacao_igualdade} para sua função objetivo através da parametrização do conjunto factível proposta em \eqref{parametrizacao_de_S}. Como as condições de otimalidade para um problema de otimização irrestrita já são conhecidas, será possível utilizá-las para determinar as condições para o problema \eqref{problema_geral_minimizacao_igualdade}. Para tanto, vejamos que os problemas \eqref{problema_geral_minimizacao_igualdade} e \eqref{problema_minimizacao_irrestrita_phi} são equivalentes.

\begin{prop}
$\gamma^{*}$ é um minimizador local (global) de $\varphi$ em $\RR^{n-m}$ se, e somente se, $x^{*} := \tilde{x} + Z\gamma^{*}$ é um minimizador local (global) de \eqref{problema_geral_minimizacao_igualdade}.
\end{prop}
\begin{proof}
Seja $\gamma^{*}$ um minimizador local (global) de $\varphi$ em $\RR^{n-m}$. Então,
\[
f(\tilde{x} + Z\gamma^{*} ) = \varphi (\gamma^{*}) \leq \varphi (\gamma) = f(\tilde{x} + Z\gamma) ,
\]
para todo $\gamma \in \RR^{n-m}$. Logo, chamando $x^{*}:=\tilde{x} + Z\gamma^{*}$, temos
\[
f(x^{*}) \leq f(x),
\]
para todo $x \in \RR^{n}$, com $x^{*} \in S$. Portanto, $x^{*}$ é um minimizador local (global) de \eqref{problema_geral_minimizacao_igualdade}.

Reciprocamente, suponhamos que $x^{*}=\tilde{x} + Z\gamma^{*}$ é um minimizador local (global) de \eqref{problema_geral_minimizacao_igualdade}. Então,
\[
...
\]
para todo $\gamma \in \RR^{n-m}$.

Portanto, $\gamma^{*}$ é um minimizador local (global) de $\varphi$ em $\RR^{n-m}$.
\end{proof}

Agora, como \eqref{problema_minimizacao_irrestrita_phi} trata de um problema irrestrito, a condição necessária de primeira ordem é
\[ \label{condicao_1_ordem_phi}
\nabla \varphi(x^{*}) = 0.
\]

Por \eqref{funcao_phi_restricoes_igualdade} e definindo a função $g: \RR^{n-m} \rightarrow \RR^{n}$, em que $g(\gamma) = \tilde{x} + Z\gamma$, temos que $\varphi(\gamma) = f(g(\gamma))$. Aplicando a regra da cadeia para calcular sua derivada, obtemos
\[
...
Desse modo, da condição de primeira ordem \eqref{condicao_1_ordem_phi}, resulta que
\[
...
Z^{T} \nabla f(x^{*}) = 0,
\]
ou seja, que $\nabla f(x^{*})$ seja ortogonal a $\mathcal{N}(A)$. Logo, pelas considerações feitas anteriormente, temos que $\nabla f(x^{*}) \in Im(A^{T})$, em outras palavras, $\nabla f(x^{*})$ deve ser uma combinação linear das linhas de $A$. Portanto, existe $\lambda^{*} \in \RR^{m}$ tal que
\[ \label{condicao_1_ordem_gradiente}
\nabla f(x^{*}) = A^{T} \lambda^{*} .
\] 

Observe que \eqref{condicao_1_ordem_f} e \eqref{condicao_1_ordem_gradiente} são equivalentes. Com efeito, se \eqref{condicao_1_ordem_f} se verifica, isso implica que $\nabla f(x^{*}) \in \mathcal{N}(A)^{\bot} = Im(A^{T})$ e portanto \eqref{condicao_1_ordem_gradiente} é verdadeiro. Reciprocamente, se \eqref{condicao_1_ordem_gradiente} ocorre, temos que, pela \Cref{prop:complemento_ortogonal_nucleo}, $Z^{T}\nabla f(x^{*}) = Z^{T}(A^{T} \lambda^{*}) = 0$.

Em vista disso, se $x^{*}$ é um minimizador local de \eqref{problema_geral_minimizacao_igualdade}, então, por \eqref{condicao_1_ordem_gradiente}, existe $\lambda^{*} \in \RR^{m}$ tal que $(x^{*}, \lambda^{*})$ é solução do seguinte sistema de $(n+m)$ equações
...
O vetor $\lambda^{*} \in \RR^{m}$ é chamado vetor de \emph{multiplicadores de Lagrange} associado a $x^{*}$.

\subsubsection{Condições Necessárias e Suficientes de Segunda Ordem}

A condição necessária de segunda ordem para uma solução do problema irrestrito \eqref{problema_minimizacao_irrestrita_phi} é
\[ \label{hessiana_phi}
\nabla^{2} \varphi (\gamma^{*}) \succcurlyeq 0 \quad \text{(semidefinida positiva)}
\]

Aplicando a regra da cadeia em $\nabla \varphi(\gamma) = Z^{T}\nabla f(\tilde{x} + Z\gamma)$, obtemos
\[
\nabla^{2} \varphi(\gamma) = Z^{T} \nabla f(\tilde{x} + Z\gamma)Z ,
\]
e \eqref{hessiana_phi} implica na condição necessária de segunda ordem para que $x^{*}$ seja minimizador local de \eqref{problema_geral_minimizacao_igualdade}, que é
\[
Z^{T} \nabla f(x^{*})Z \succcurlyeq 0.
\]
...
\[
y^{T}\nabla f(x^{*})y \succcurlyeq 0 \quad \text{para todo} \quad y \in \mathcal{N}(A) .
\]

Analogamente, a partir das condições suficientes para o problema \eqref{problema_minimizacao_irrestrita_phi}, obtemos as seguintes condições suficientes de segunda ordem:

Se $x^{*} \in \RR^{n}$ verifica $Ax^{*}=b$ e 
\begin{enumerate}
	\item[(i)] $Z^{T}\nabla f(x^{*}) =0$
	\item[(ii)] $Z^{T}\nabla f(x^{*})Z \succ 0$ (definida positiva),
\end{enumerate}	
então $x^{*}$ é um minimizador local de \eqref{problema_geral_minimizacao_igualdade}.

No exemplo a seguir, as condições de otimalidade nos permitem determinar a expressão geral para uma solução do problema de minimizar $f(x) = \Vert x \Vert$ sujeita a restrições de igualdade.
...
Vejamos um exemplo em que a função objetivo é uma quadrática.
