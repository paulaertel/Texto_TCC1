...
Ademais, nossa intenção é estudar a técnica SVM aplicada na classificação de dados. Neste caso, na fase de treinamento é fornecido um conjunto de dados rotulados da forma $(x^{i}, y_{i})$, em que $x^{i}$ representa o vetor de características (ou atributos) e $y_{i}$ corresponde à saída ou rótulo, e a partir dos quais a técnica SVM irá gerar um classificador capaz de predizer a saída correta de novos dados. É importante salientar que trataremos aqui do problema de classificação binário, isto é, quando existem apenas duas saídas possíveis, como por exemplo $1$ ou $-1$, $0$ ou $1$, \emph{vermelho} ou \emph{azul}. No entanto é possível generalizar tal técnica para problemas que envolvam mais saídas, se configurando neste caso num problema multiclasses. Na \Cref{fig:SVM_classificador} a seguir temos um esquema representando a atuação da técnica de SVM para obtenção de um classificador.

\begin{figure}[!ht] 
	\centering
	\includegraphics[width=0.70\textwidth]{representacao_tecnica_SVM}
	\caption{Atuação da técnica de SVM na obtenção de um classificador. \\ Fonte: \textcite{Lorena&Carvalho} \label{fig:SVM_classificador}}
\end{figure}

Em síntese, ao formular o problema de classificação utilizando a técnica SVM veremos que este consiste no seguinte problema de programação quadrática convexa e com restrições lineares
\[ \label{problema_geral_SVM_introducao}
\begin{aligned}
\min_{w,b} & \quad f(w) \\
\text{s.a.} &  \quad g(w,b) \leq 0, \end{aligned}
\]
em que as funções $f:\RR^{n+1} \rightarrow \RR$ e $g:\RR^{n+1} \rightarrow \RR^{m}$ são continuamente diferenciáveis. Como \eqref{problema_geral_SVM_introducao} é um problema de otimização convexa com restrições lineares, desenvolveremos nos \Cref{chap:conceitos_de_otimizacao,chap:otimizacao_convexa} seu embasamento matemático. Além disso, o objetivo ao resolver tal problema é obter um classificador a partir da solução $(w, b)$ \cite{Evelin2017}. Veremos mais adiante que a solução encontrada fornece um hiperplano definido pela equação $w^{T}x + b = 0$, que será justamente o classificador que desejamos encontrar.

AQUI ABORDAR UM POUCO DA ORGANIZAÇÃO DO TRABALHO.
