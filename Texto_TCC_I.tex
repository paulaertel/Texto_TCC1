...
Uma classe especial de problemas de otimização se refere ao caso em que $\Omega$ é um conjunto poliedral. Assim, dizemos que um conjunto viável é \emph{poliedral} quando ele pode ser representado como o conjunto das soluções de um sistema finito de equações e inequações lineares. Por exemplo,
\[
\Omega = \{ x \in \RR^{n} \mid  Ax \leq b, Mx = r\},
\]
em que $A \in \RR^{mxn}$, $M\in \RR^{pxn}$, $b \in \RR^{m}$, $r \in \RR^{p}$. Neste contexto, dizemos que $g:\RR^{n} \rightarrow \RR^{m}$, com $g(x) = Ax-b$, e $h:\RR^{n}\rightarrow \RR^{p}$, com $h(x) = Mx - r$, são funções afins.
...
\[
\begin{aligned} \label{eq:1_ponto_estacionario}
\nabla f(x^{*})+\sum_{i=1}^{m} \mu_{i}^{*} \nabla g_{i}(x^{*})+\sum_{i=1}^{p} \lambda_{i}^{*} \nabla h_{i}(x^{*}) & =0,  \\ 
g(x^{*}) \leq 0, h(x^{*}) & =0,   \\ \mu_{i}^{*} & \geq 0, \quad i= 1, \ldots , m,   \\ 
\lambda_{i}^{*} g_{i}(x^{*}) & =0, \quad i= 1, \ldots , m.  
\end{aligned}
\]
\end{defi}
%ver como enumerar cada condição separadamente, pois não está aceitando
As condições em \eqref{eq:1_ponto_estacionario} são conhecidas como condições de Karush-Kuhn-Tucker (KKT). Assim, um ponto é dito estacionário quando satisfaz as condições KKT. Sob determinadas hipóteses de qualificação, as condições KKT são satisfeitas em um ponto que seja minimizador.
...
\text{s.a} & \quad g(x)=-x_{1}^{3} + x_{2} \leq 0 , \\
& \quad h(x) = -x_{2} \leq 0.
\end{aligned}
\]
...
No entanto, 
\[
\nabla f(x^{*}) = \left( \begin{array}{cc} 1 \\ 0 \end{array} \right) , \nabla g(x^{*}) = \left( \begin{array}{cc} 0 \\ 1 \end{array} \right) \quad \text{e} \quad \nabla h(x^{*}) = \left( \begin{array}{cc} 0 \\ -1 \end{array} \right) ,
\]
e, portanto, as condições KKT não são satisfeitas, pois não existem $\mu^{*} \in \RR$ E $\lambda^{*} \in \RR$ tais que 
\[
\left( \begin{array}{cc} 1 \\ 0 \end{array} \right) + \mu^{*} \left( \begin{array}{cc} 0 \\ 1 \end{array} \right) + \lambda^{*} \left( \begin{array}{cc} 0 \\ -1 \end{array} \right) = \left( \begin{array}{cc} 0 \\ 0 \end{array} \right) .
\]
