...
\begin{figure}[!ht] 
	\centering
	\includegraphics[width=0.60\textwidth]{nucleoA_paralelo_S}
	\caption{ Núcleo de uma matriz. \\ Fonte: \textcite{Ana1994} \label{fig:nucleoA_paralelo_S}}
\end{figure}
...
\[
\Omega = \{ x\in \RR^{n} \mid w_{i}^{T}x \leq c_{i} \ \text{para todo} \ i \in \{ 1,2, \ldots , m \} \} .
\]

Desse modo, cada uma das $m$ desigualdades $w_{i}^{T}x \leq c_{i}$ define em $\RR^{n}$ um semiespaço. O hiperplano divisor é $w_{i}^{T}x = c_{i}$ e o semiespaço definido é aquele que está do lado contrário à direção apontada pelo vetor $w_{i}$. Assim, a região $\Omega$ consiste na instersecção dos $m$ semiespaços. Em alguns casos, como representado na \Cref{fig:regiao_factivel_poliedro}, $\Omega$ é um conjunto \emph{poliedral} em $\RR^{n}$.
...
Dizemos que um conjunto é \emph{poliedral} quando ele pode ser representado como o conjunto de soluções de um sistema finito de equações e inequações lineares. Por exemplo:
\[
D = \{ x\in \RR^{n} \mid Ax=b, \ Wx \leq c \} ,
\] 
em que $A\in \RR^{m\times n}$, $W\in \RR^{p\times n}$, $b\in \RR^{m}$ e $c\in \RR^{p}$. Neste contexto, dizemos que $h:\RR^{n} \rightarrow \RR^{m}$, $h(x) = Ax-b$, é uma \emph{função afim}. Assim como $g:\RR^{n} \rightarrow \RR^{p}$, dada por $g(x)=Wx-c$.
\end{defi}


Por conseguinte, um conceito fundamental para formular as condições de otimalidade e que fornece uma grande quantidade de informações é o de \emph{restrição ativa}.

\begin{defi}
Dado $\xbar \in \Omega$, dizemos que a restrição de desigualdade $w_{i}^{T}\bar{x} \leq c_{i}$ que corresponde ao índice $i\in \{ 1,2, \ldots , m\}$ é \emph{ativa} no ponto $\xbar$ quando $w_{i}^{T}\bar{x} = c_{i}$. Caso $w_{i}^{T}\bar{x} < c_{i}$ dizemos que a restrição é \emph{inativa} em $\xbar$. 
\end{defi} 
...
Considere $x\in \Omega$ tal que $r(x) = p$ com $0<p \leq m$. Um vetor $d \in \RR^{n}$ é uma direção factível em $x$ se, e somente se, $w_{i}^{T}d \leq 0$ para todo $i \in \mathcal{I} (x)$.
\end{teo}
\begin{proof}
Seja $\alpha >0$ e suponhamos que $d\in \RR^{n}$ é uma direção factível em $x$. Então, $x+\alpha d \in \Omega$, o que ocorre se, e somente se, $W(x+\alpha d) \leq c$, ou seja, $w_{i}^{T}(x+\alpha d) \leq c_{i}$, para todo $i \in \mathcal{M} \coloneqq \{ 1,2, \ldots ,m \}$. 
...
Reciprocamente, suponhamos que $w_{i}^{T}d \leq 0$ para todo $i \in \mathcal{I} (x)$. Assim, se $i \in \mathcal{I} (x)$, temos que $w_{i}^{T} x = c_{i}$, implicando que $w_{i}^{T}(x+\alpha d) \leq c_{i}$.
Por outro lado, se $i \notin \mathcal{I}(x)$, temos que $w_{i}^{T} x < c_{i}$. Neste caso, é preciso analisar duas situações:
\begin{enumerate}
	\item[(i)] Se $w_{i}^{T}d \leq 0$, então $w_{i}^{T}(x + \alpha d) \leq c_{i}$;

	\item[(ii)]Se $w_{i}^{T}d >0$ podemos calcular o valor de $\alpha$ de modo que $w_{i}^{T}(x+\alpha d) = c_{i}$. Desse modo, temos que
	\[
	w_{i}^{T}x + \alpha w_{i}^{T}d = c_{i} ,
	\]
	e resolvendo para $\alpha$,
	\[ \label{eq:1_teo_direcoes_factiveis_restricao_desigualdade}
	\alpha = \dfrac{c_{i} - w_{i}^{T}x}{w_{i}^{T}d}.
	\]
	Então, para $\alpha$ satisfazer todas as restrições $i \in \mathcal{M}-\mathcal{I}(x)$, é preciso tomar o mínimo de \eqref{eq:1_teo_direcoes_factiveis_restricao_desigualdade}. Assim, se definimos
	\[
	\tilde{\alpha} = \min_{i \in \mathcal{M}-\mathcal{I}(x) } \left( \dfrac{c_{i} - w_{i}^{T}x}{w_{i}^{T}d} \right) ,
	\]
...
Além disso, interessados em saber quanto podemos andar sobre uma direção factível $d$ a partir de $x \in \Omega$, precisamos determinar o maior valor de $\alpha$ tal que $w_{i}^{T}(x + \alpha d) \leq c_{i}$, para todo $i\in \mathcal{M}$, o que equivale a determinar o menor valor de $\alpha$ para o qual $w_{i}^{T}(x+\alpha d) = c_{i}$, para algum $i\in \mathcal{M}$. Este valor é dado na demonstração do \Cref{teo:direcoes_factiveis_restricao_desigualdade}, em que se $i$ é tal que $w_{i}^{T}d \leq 0$, $\alpha$ pode ser arbitrariamente grande. E no caso em que $i$ é tal que $w_{i}^{T}d >0$, o valor procurado é
\[
\tilde{\alpha} = \min_{i \in \mathcal{M}-\mathcal{I}(x) } \left( \dfrac{c_{i} - w_{i}^{T}x}{w_{i}^{T}d} \right) .
\]
...
\[
\nabla f(x^{*}) = \sum_{k=1}^{r(x^{*})} \lambda_{k} w_{i_{k}} \quad \text{e} \quad \lambda_{k} \leq 0, \ 1\leq k\leq r(x^{*}),
\]
ou, equivalentemente
\[ \label{eq:condicao_necessaria_1ordem_restricao_desigualdade}
\nabla f(x^{*}) = W_{\mathcal{I}}^{T} \boldsymbol{\lambda}, \quad \boldsymbol{\lambda} \in \RR^{r(x^{*})}
\]
e
\[
...
\begin{enumerate}
\item[(i)] $\nabla f(x^{*}) \neq W_{\mathcal{I}}^{T} \boldsymbol{\lambda} \ \text{para todo} \ \boldsymbol{\lambda} \in \RR^{r(x^{*})}$.
...
\item[(ii)] $\nabla f(x^{*}) = W_{\mathcal{I}}^{T} \boldsymbol{\lambda}$, mas existe $j$ tal que $\lambda_{j} >0$.

Se $r(x^{*}) = 1$ e $\mathcal{I} = \{ i_{1} \}$, então $\nabla f(x^{*}) = \lambda_{1} w_{i_{1}}$ e $\lambda_{1} >0$. Tomando $d=-\nabla f(x^{*})$ temos que
\[
w_{i_{1}}^{T}d = -\lambda_{1} w_{i_{1}}^{T}w_{i_{1}} = -\lambda_{1} \Vert w_{i_{1}} \Vert^{2} < 0.
\]
...
Agora, se $2\leq r(x^{*}) \leq n$, denotamos por $\tilde{W_{\mathcal{I}}}$ a matriz obtida retirando a linha $w_{i_{j}}$ correspondente ao multiplicador $\lambda_{j} >0$. Consideramos $d=P_{\mathcal{N}(\tilde{W_{\mathcal{I}}})} (-\nabla f(x^{*}))$, em que $P_{\mathcal{N}(\tilde{W_{\mathcal{I}}})}$ é o operador projeção ortogonal sobre $\mathcal{N}(\tilde{W_{\mathcal{I}}})$. Então, temos que
\[
(-\nabla f(x^{*}) -d)^{T}d =0,
\]
implicando em
...
\[
\nabla f(x^{*}) = \lambda_{1} w_{i_{1}} + \lambda_{2} w_{i_{2}} + \ldots + \lambda_{j} w_{i_{j}} + \ldots + \lambda_{r(x^{*})} w_{i_{r(x^{*})}}
\]
...
Então,
\[
\nabla f(x^{*})^{T}d = \lambda_{j} w_{i_{j}}^{T} d.
\]
Usando \eqref{eq:2_condicao_necessaria_1ordem_restricao_desigualdade} e o fato de $\lambda_{j} >0$, obtemos
\[
w_{i_{j}}^{T} d <0.
\]
...
\begin{enumerate}
	\item[(i)] Existe $\boldsymbol{\lambda} \in \RR^{r(x^{*})}$ tal que $\nabla f(x^{*}) = W_{\mathcal{I}}^{T} \boldsymbol{\lambda}$ e $\lambda_{i} \leq 0$ para todo $i \in \mathcal{I}(x^{*})$;

	\item[(ii)] Para todo $y \in \mathcal{N}(W_{\mathcal{I}})$ temos que $y^{T} \nabla^{2} f(x^{*})y \geq 0$.
\end{enumerate}
\end{teo}
\begin{proof}
Perceba que se $x^{*}$ é minimizador local de \eqref{problema_geral_minimizacao_desigualdade}, então pelo \Cref{teo:condicao_necessaria_1ordem_restricao_desigualdade}, existe $\boldsymbol{\lambda} \in \RR^{r(x^{*})}$ tal que 
\[
\nabla f(x^{*}) = W_{\mathcal{I}}^{T} \boldsymbol{\lambda} \ \text{e} \ \lambda_{i} \leq 0,
\] 
...
\]
com $x\in \RR^{n}$. Emtão a função $f$ é uma função convexa.
\end{prop}
\begin{proof}
