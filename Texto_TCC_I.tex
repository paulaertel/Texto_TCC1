...
Tendo em vista que a função objetivo dos problemas de SVM são funções quadráticas, é necessário avaliar se a função quadrática é convexa. Tal resultado é apresentado a seguir e sua demonstração decorre do \Cref{teo:hessiana_convexidade}.

%\cite{Gondzio2018}
\begin{teo}
Seja $C \in \RR^{n}$ um conjunto convexo e $Q$ uma matriz quadrada e simétrica. Seja $f:C \rightarrow \RR$ tal que $f(x) = x^{T}Qx$ é uma função quadrática. Então, $f$ é convexa se, e somente se, $Q$ é semidefinida positiva.
\end{teo}
\begin{proof}
...
Para problemas de minimização irrestrita em que a função objetivo $f$ é quadrática, se $x^{*}$ é mínimo local de $f$ então, pelas condições necessárias de otimalidade, \Cref{teo:condicao_necessaria_1_ordem,teo:condicao_necessaria_2_ordem}, devemos ter
\[
\nabla f(x^{*})= Qx^{*}-b =0 \quad \text{e} \quad \nabla^{2} f(x^{*})=Q \succ 0.
\]
Assim, se $Q$ é semidefinida positiva, pelo \Cref{teo:hessiana_convexidade} $f$ é convexa, e pelo \Cref{teo:convexidade_e_minimizador} qualquer ponto $x^{*}$ que satisfaça a condição de otimalidade de primeira ordem 
\[ \label{eq:condicao_1_ordem_quadratica_convexidade}
\nabla f(x^{*})= Qx^{*}-b =0
\] 
é um minimizador global de $f$. Por outro lado, pode não existir solução para a equação \eqref{eq:condicao_1_ordem_quadratica_convexidade} se $Q$ é singular. No entanto, se $Q$ é definida positiva então é invertível, e a equação \eqref{eq:condicao_1_ordem_quadratica_convexidade} pode ser resolvida de modo único e o vetor
\[
x^{*} = Q^{-1}b
\]  
