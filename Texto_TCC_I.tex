...
x_{2} = 1 - 2x_{1} ,
\]
e escolhendo $x_{1} =1$ obtemos $x_{2} = -1$, isto é, $\tilde{x} = (1,-1)$. E assim, podemos reescrever o conjunto factível $\Omega$ como proposto em \eqref{parametrizacao_de_Omega}:
\[
...
Assim, para determinar a solução do problema \eqref{exem:parametrizacao_omega} utilizamos o fato de haver uma equivalência entre sua solução e a solução do problema irrestrito \eqref{exem:parametrizacao_omega_funcao_phi}. Essa equivalência entre as soluções é garantida pela proposição a seguir.
...
\]
para todo $\boldsymbol{\gamma} \in B( \boldsymbol{\gamma}^{*} , \delta ) \cap \RR^{n-m}$, em que $\delta \coloneqq \dfrac{\varepsilon}{\Vert Z \Vert}$. Logo, chamando $x^{*}\coloneqq \tilde{x} + Z\boldsymbol{\gamma}^{*} \in \Omega$, temos para $\boldsymbol{\gamma} \in B( \boldsymbol{\gamma}^{*} , \delta ) \cap \RR^{n-m}$ que
\[
...
\end{aligned}
\]
em que na linha \eqref{eq:teste} usamos a consistência de norma. Portanto, $x\in B(x^{*}, \varepsilon ) \cap \Omega$, e assim,  $x^{*}$ é um minimizador local (global) de \eqref{problema_geral_minimizacao_igualdade}.
...
y^{T}\nabla^{2} f(x^{*})y > 0.
\]
Portanto, se $x^{*} \in \Omega$ satisfaz \eqref{eq:3_condicao_suficiente_igualdade} e \eqref{eq:4_condicao_suficiente_igualdade}, então ele é um minimizador local de \eqref{problema_geral_minimizacao_igualdade}. Assim, podemos enunciar o seguinte teorema.
...
Para $x^{*} \in \Omega$, com $1 \leq r(x^{*}) \leq m$, vamos denotar por $\mathcal{I}(x^{*}) = \{ i_{1}, i_{2}, \ldots , i_{r(x^{*})} \}$ o conjunto dos índices das restrições ativas em $x^{*}$, e por $W_{\mathcal{I}} \in \RR^{r(x^{*})\times n}$ a submatriz de $W$ cujas linhas são as que tem índices em $\mathcal{I}(x^{*})$ e 
\[
...
\begin{obs}
Novamente, a análise dependerá das restrição ativas em $\xbar$. Se $r(\xbar) =0$, o ponto está no interior de $\Omega$ e as condições necessárias e suficientes são as mesmas do caso em que o problema é irrestrito, pois qualquer direção é factível nesse ponto.
\end{obs}
...
\begin{enumerate}
	\item[(i)] $\nabla f(x^{*}) + W_{\mathcal{I}}^{T} \boldsymbol{\mu} =0$ com $\mu_{k} \geq 0$ para todo $i_{k} \in \mathcal{I}(x^{*})$, e 
...
\]
em que $\boldsymbol{\mu}_{\mathcal{J}}$ é vetor formado pelas componentes $\mu_{k}$ de $\boldsymbol{\mu}$ que possuem índice em $\mathcal{J} = \{ k\in \{ 1,2, \ldots , r(x^{*}) \} \mid \mu_{k} >0 \}$. Desse modo, temos que $x^{*} \in \Omega$ satisfaz
\[
...
Nesta seção abordaremos o problema geral de otimização, isto é, com restrições de igualdade e desigualdade. Em vista disso, utilizando os argumentos das \Cref{section:restricoes_igualdade,section:restricoes_desigualdade}, pretendemos obter as condições de otimalidade de Karush-Kuhn-Tucker e as condições necessárias e suficientes de segunda ordem para o problema
...
Por conseguinte, as caracterizações das direções factíveis propostas nos \Cref{teo:direcao_factivel_igualdade,teo:direcoes_factiveis_restricao_desigualdade} nos permitem enunciar o seguinte teorema. 
...
Nos teoremas a seguir vamos considerar $\mathcal{I}(x^{*})$ e $\mathcal{J}(x^{*})$ como definidos anteriormente e denotaremos por $W_{\mathcal{J}}$ a submatriz de $W$ cujas linhas são as que têm os índices em $\mathcal{J}(x^{*})$, por $c_{\mathcal{J}} \in \RR^{s(x^{*})}$ o vetor formado pelas componentes de $c$ correspondentes a $\mathcal{J}(x^{*})$ e por $B \in \RR^{r(x^{*})\times n}$ a matriz dada por
\[
...
Se $x^{*}$ é minimizador local de \eqref{problema_geral_minimizacao_igualdade_desigualdade}, e $\posto (B)= r(x^{*})$, então existe $\boldsymbol{\lambda} \in \RR^{m}$ e $\boldsymbol{\mu} \in \RR^{s(x^{*})}$ tais que
\[ \label{eq:1_teo_KKT}
\nabla f(x^{*}) + A^{T}\boldsymbol{\lambda} + W^{T}_{\mathcal{J}} \boldsymbol{\mu} = 0
\]
e
\[ \label{eq:2_teo_KKT}
\mu_{k} \geq 0 \ \text{para todo} \ k \ \text{tal que} \ 1 \leq k \leq s(x^{*}).
\]
\end{teo}
\begin{proof}
Por demonstrar.
...
As condições \eqref{eq:1_teo_KKT} e \eqref{eq:2_teo_KKT} são conhecidas como condições de Karush-Kuhn-Tucker (KKT). A partir delas podemos definir \emph{ponto estacionário}.

\begin{defi} 
Um ponto $x^{*} \in \RR^{n} $ é um \emph{ponto estacionário} do problema \eqref{problema_geral_minimizacao_igualdade_desigualdade} (ou ainda, um \emph{ponto de Karush-Kuhn-Tucker(KKT)}), quando $x^{*} \in \Omega$ e existem $\boldsymbol{\lambda}^{*} \in \RR^{m}$ e $\boldsymbol{\mu}^{*} \in \RR^{p}$, tais que as condições \eqref{eq:1_teo_KKT} e \eqref{eq:2_teo_KKT} são satisfeitas. Os elementos $\boldsymbol{\mu}^{*}$ e $\boldsymbol{\lambda}^{*}$ são chamados os \emph{multiplicadores de Lagrange} associados ao ponto estacionário $x^{*}$.
\end{defi}
Em outras palavras, $x^{*}$ é dito estacionário quando satisfaz as condições KKT. 

É importante observar que se não for verificada a hipóstese de a matriz $B$ ser posto completo, podemos ter minimizadores que não cumprem KKT, o que dificulta a caracterização de tais pontos. Vejamos a seguir um exemplo que justifica essa afirmação.
...
\begin{enumerate}
	...
	\[
	\nabla f(x^{*}) + A^{T}\boldsymbol{\lambda} + W^{T}_{\mathcal{J}} \boldsymbol{\mu} =0 \ \text{e} \ \mu_{k} \geq 0 \ \text{para todo} \ k\in \{ 1,2, \ldots , s(x^{*}) \} ;
	\]
	\item[(ii)] $y^{T}\nabla^{2} f(x^{*})y \geq 0$ para todo $y\in \mathcal{N}(B)$.
\end{enumerate}	
\end{teo}
\begin{proof}
Por demonstrar.
