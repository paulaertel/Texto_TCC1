...
Situações reais dificilmente envolvem problemas cujos dados são linearmente separáveis. Em vista disso, faz-se necessário estender os conceitos e resultados estudados nas SVMs lineares de margem rígida para o caso de SVM com margem flexível, quando os dados não são linearmente separáveis. Para tanto, considere um conjunto de dados não-linearmente sepavável como da \Cref{fig:dados_linearmente_separaveis_e_nao_separaveis:b}, isto é, não existe um hiperplano separador. 
...
\]
Passando ao limite quando $k \rightarrow \infty $, e usando o Teorema do Sanduíche conluímos que
...
Para os resultados que vem a seguir vamos utilizar os \Cref{teo:Taylor_primeira_ordem,teo:Taylor_segunda_ordem}, que fornecem a expansão de Taylor de primeira e segunda ordem.

%determinar bibliografia em que estão demonstrados para citar.
...
f(x^{*}) \leq f(x^{*}+td),
\]
para todo $t\in (0,\varepsilon ]$. Pela diferenciabilidade de $f$ em $x^{*}$, aplicando o \Cref{teo:Taylor_primeira_ordem}, podemos escrever 
\[
f(x^{*}+td) = f(x^{*}) + t\nabla f(x^{*})^{T}d + o(t),
\] 
...
0 \leq f(x^{*}+td) - f(x^{*}).
\]
Aplicando o \Cref{teo:Taylor_segunda_ordem},
\[
...
AQUI ACRESCENTAR DEFINIÇÃO DE DEFINIDA POSITIVA E SEMIDEFINIDA POSITIVA.

É possível obter, a partir condição necessária de segunda ordem, a condição suficiente de segunda ordem, isto é, condição que implica que $x^{*}$ é minimizador local estrito, bastando para isso pedir a definição positiva da Hessiana.
...
\]
Aplicando o \Cref{teo:Taylor_segunda_ordem} em torno de $x^{*}$, temos
...
\end{aligned} \]
\end{exem}
Para um ponto $x^{*}$ ser minimizador estrito de \eqref{exem:verificar_condicao_suficiente_irrestrito} ele deve satisfazer as condições suficientes de segunda ordem do \Cref{teo:condicao_suficiente_irrestrito}, que são
...
\]
e, para todo $x \in \RR^{2}-\{ 0\}$, temos que
\[
...
\]
Portanto, $\bar{x} = (1/3, -1/3 )$ é minimizador estrito do problema \eqref{exem:verificar_condicao_suficiente_irrestrito}.
...
\Omega = \{ x \in \RR^{n} \mid x=\tilde{x} + Z\boldsymbol{\gamma} , \ \boldsymbol{\gamma} \in \RR^{n-m} \ \text{e} \ A\xbar =b \} .
\]

\subsubsection{Condições Necessárias de Primeira Ordem}

Como vimos anteriormente, modo geral, se um ponto é solução de um problema de otimização então deve satisfazer determinadas propriedades, que são chamadas de condições de otimalidade. Nesta seção abordaremos a condição necessária de primeira ordem para o problema de minimização com restrições de igualdade. Para obter esta condição utilizaremos a parametrização do conjunto factível proposta em \eqref{parametrizacao_de_S}, transferindo as restrições de \eqref{problema_geral_minimizacao_igualdade} para sua função objetivo. Com isso obtemos um novo problema de minimização irrestrita, para o qual as condições necessárias de primeira e segunda ordem já são conhecidas.
...
Agora, como \eqref{problema_minimizacao_irrestrita_phi} é um problema de minimização irrestrito, o \Cref{teo:condicao_necessaria_1_ordem} nos diz que a condição necessária de primeira ordem para algum $x^{*} \in \RR^{n}$ é
...
\begin{obs}
A matriz $\tilde{A}$ é também chamada de pseudo-inversa de Moore-Penrose e geralmente denotada por $A^{\dag}$. CITAR MEYER
\end{obs}
...
\begin{enumerate}
	...
	= & b,
	\end{aligned} \]
pois $AZ=0$ pelo fato de cada coluna de $Z$ pertencer ao $\mathcal{N}(A)$ e portanto, ser ortogonal a $Im(A^{T})$.
...
\end{aligned}
\]
em que $x \in \RR^{n}$ e $A \in \RR^{m\times n}$. Como na seção anterior, estamos interessados nesse primeiro momento em analisar a região de factibilidade e determinar as direções factíveis, que são aquelas em que há espaço para se movimentar dentro da região $\Omega$. 
...
Desse modo, cada uma das $m$ desigualdades $a_{i}^{T}x \leq b_{i}$ define em $\RR^{n}$ um semiespaço. O hiperplano divisor é $a_{i}^{T}x = b_{i}$ e o semiespaço definido é aquele que está do lado contrário à direção apontada pelo vetor $a_{i}$. Assim, a região $\Omega$ consiste na instersecção dos $m$ semiespaços. Em alguns casos, como representado na \Cref{fig:regiao_factivel_poliedro}, $\Omega$ é um poliedro em $\RR^{n}$.
...
Um conceito fundamental que fornece uma grande quantidade de informações é o de \emph{restrição ativa}.
...
O conjunto das direções factíveis a partir de $x$ depende das restrições ativas nesse ponto, pois são elas que restringem o domínio de factibilidade na vizinhança de $x$, enquanto que as restrições inativas não possuem influência na vizinhança de $x$. Um exemplo é representado na \Cref{fig:restricoes_inativas}, em que é possível perceber que a solução $x^{*}$ não depende das restrições inativas $g_{2}, g_{3}$.

\begin{figure}[!h] 
	\centering
	\includegraphics[width=0.45\textwidth]{restricoes_inativas}
	\caption{Restrições ativas e inativas. \\ Fonte: \textcite{Luenberger:2016} \label{fig:restricoes_inativas}}
\end{figure}

Ademais, a cada $x\in \Omega$ podemos associar um número $r(x)$, com $0\leq r(x) \leq m$, que representa a quantidade de restrições ativas em $x$. Assim, considere $x\in \Omega$ tal que $r(x) = p$ com $0<p \leq m$. O \Cref{teo:direcoes_factiveis_restricao_desigualdade} nos fornece uma caracterização das direções factíveis em $x$.
...
\end{teo}
\begin{proof}
Seja $\alpha >0$ e suponhamos que $d\in \RR^{n}$ é uma direção factível em $x$. Então, $x+\alpha d \in \Omega$, o que ocorre se, e somente se, $A(x+\alpha d) \leq b$, ou seja, $a_{i}^{T}(x+\alpha d) \leq b_{i}$, para todo $i \in \mathcal{M} \coloneqq \{ 1,2, \ldots ,m \}$. 
...
f(\xbar + td) < f(\xbar).
\]
Pelo \Cref{teo:Taylor_primeira_ordem}, temos
...
Suponhamos agora que $\nabla f(\xbar)^{T}d <0$. Temos que, pelo \Cref{teo:Taylor_primeira_ordem},
\[
...
Assim, a partir de um ponto $x\in \Omega$, estamos interessados em saber se existem direções de descida factíveis, isto é, direções factíveis tais que
\[ \label{eq:caracterizacao_direcao_descida}
\nabla f(x)^{T}d < 0,
\]
pois se existe uma direção $d$ que satisfaz \eqref{eq:caracterizacao_direcao_descida}, decorre do \Cref{lema:direcao_de_descida} e da \Cref{defi:direcao_de_descida} que $x$ não é minimizador local do problema \eqref{problema_geral_minimizacao_desigualdade}.
...
\[
b_{\mathcal{I}} = \begin{bmatrix*} b_{i_{1}} \\ b_{i_{2}} \\ \vdots \\ b_{i_{r(x^{*})}} \end{bmatrix*} .
\]
...
\]
e portanto, $d$ é uma direção de descida. Veja \Cref{fig:projecao_sobre_nucleo_teo_condicao_1ordem_desigualdade}.
...
\end{teo}
\begin{proof}
A prova do item $(i)$ é análoga a demonstração do \Cref{teo:condicao_necessaria_1ordem_restricao_desigualdade}.
...
Finalmente, trataremos do caso mais geral do problema de minimização de funções sujeitas a restrições lineares, isto é, com restrições de igualdade e desigualdade. Nesta seção, utilizando os argumentos das \Cref{section:restricoes_igualdade} e \Cref{section:restricoes_desigualdade} desenvolveremos as condições necessárias e suficientes para o problema
...
Por conseguinte, a partir das caracterizações das direções factíveis propostas na \Cref{section:restricoes_igualdade} e \Cref{section:restricoes_desigualdade}, concluímos que, dado um ponto factível $x$, $d\in \RR^{n}$ é uma direção factível em $x$ se, e somente se, $Ad =0$ e $w_{j}^{T}d \leq 0$ para todo $j\in \mathcal{J}(x)$.
...
\section{Otimização Convexa} \label{section:otimizacao_convexa}
...
Esta definição pode ser interpretada geometricamente afirmando que um conjunto é convexo se, dados dois pontos no conjunto, cada ponto no segmento de linha que une esses dois pontos também for um membro do conjunto. A \Cref{fig:conjuntos_convexos} ilustra a noção de convexidade de conjuntos.
...
\begin{subfigure}[!h]{0.43\textwidth}
	...
	\caption{Função estritamente convexa. \label{fig:funcao_estritamente_convexa}}
\end{subfigure}
\begin{subfigure}[!h]{0.4\textwidth}
	\centering
...
\end{teo}
\begin{proof}
O resultado decorre imediatamente do \Cref{teo:hessiana_convexidade}, pois a Hessiana $\nabla^{2} f(x) = Q$.
\end{proof}
